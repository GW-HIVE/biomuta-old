...
def main():
	...
	in_file = config_json["loader"]["outdir"] + "/pph-final.csv"
	with open(in_file, 'r') as FR:
                csv_grid = csv.reader(FR, delimiter=',', quotechar='"')
                for row in csv_grid:
			canonical_ac, start_pos, end_pos, ref, alt = row[0],row[1],row[1],row[2],row[3]
			ann_value = row[-2] + " (probability = " + row[-1] + ")"
			sql = "INSERT INTO biomuta_protein_ann "
			sql += "(canonical_ac, start_pos, end_pos, ref, alt, ann_type, ann_name,ann_value) "
			sql += "VALUES ('%s',%s,%s,'%s','%s','%s', '%s','%s')" % (canonical_ac, start_pos, end_pos,ref, alt, "polyphen","polyphen",ann_value)
