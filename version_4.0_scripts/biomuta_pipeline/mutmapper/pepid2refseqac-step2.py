...
def main():
	...
	refseq_file = config_json["refseqfile"]
	out_file = config_json["mutmapper"]["outdir"] + "/map2refseq.ranges.csv"        
	...
	for record in Seq_iO.parse(pep_file, "fasta"):
        ...
        refseq_pep_seq_hash = {}
        for record in Seq_iO.parse(refseq_file, "fasta"):
                refseq_ac = record.id
		refseq_pep_seq_hash[refseq_ac] = record.seq.upper()


	FW = open(out_file, "w")
	FW.write("peptide_id,refseq_ac,ranges\n")
        with open(config_json["pepid2refseqac"], "r") as FR:
			...
			if row[1] in ["exact", "ignore"]:
				continue
			for refseq_ac in ac_list:
				if refseq_ac not in refseq_pep_seq_hash:
					continue
				seq1, seq2 = ens_pep_seq_hash[pep_id], refseq_pep_seq_hash[refseq_ac]
				...
				if ratio1 > 0.7 and len(range_list) <= 10:
					FW.write("%s,%s,%s\n" % (pep_id, refseq_ac, "|".join(range_list)))
	FW.close()
